-- ============================================================
-- 4 новых достижения: Дуэли +1, Кто ты из... +2, Стрики +1.
-- Плюс переупорядочиваем ord всех достижений, чтобы новые встали
-- на логичные места внутри своих категорий, а не в конец списка.
-- ============================================================

insert into public.achievements (key, title, description, icon) values
  ('perfect_duel',    'Безупречная дуэль', 'Выиграй дуэль, ответив правильно на все вопросы', '🌟'),
  ('first_persona',   'Первое знакомство',  'Пройди свой первый тест «Кто ты из...»',           '🧩'),
  ('category_master', 'Знаток темы',        'Пройди все тесты одной категории',                 '📚'),
  ('streak_14',       'Две недели подряд',  'Заходи в приложение 14 дней подряд',               '📅')
...
update public.achievements set category = 'Дуэли',        ord = 6  where key = 'perfect_duel';
update public.achievements set category = 'Соло и спринт', ord = 7  where key = 'perfect_solo';
update public.achievements set category = 'Соло и спринт', ord = 8  where key = 'sprint_ace';
update public.achievements set category = 'Соло и спринт', ord = 9  where key = 'all_categories';
update public.achievements set category = 'Кто ты из...',  ord = 10 where key = 'first_persona';
update public.achievements set category = 'Кто ты из...',  ord = 11 where key = 'all_personas';
update public.achievements set category = 'Кто ты из...',  ord = 12 where key = 'category_master';
update public.achievements set category = 'Особые',        ord = 13 where key = 'score_5000';
update public.achievements set category = 'Особые',        ord = 14 where key = 'all_modes';
update public.achievements set category = 'Особые',        ord = 15 where key = 'night_owl';
update public.achievements set category = 'Стрики',        ord = 16 where key = 'streak_7';
update public.achievements set category = 'Стрики',        ord = 17 where key = 'streak_14';
update public.achievements set category = 'Стрики',        ord = 18 where key = 'streak_30';

-- ------------------------------------------------------------
-- get_user_stats — плюс perfect_duel и category_master.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- check_achievements — плюс 4 новых условия.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- get_achievements — плюс прогресс для streak_14 (остальные три новые
-- — одношаговые, без прогресс-бара, как first_duel/real_duel/night_owl).
-- ------------------------------------------------------------
