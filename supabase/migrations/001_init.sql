...
-- ------------------------------------------------------------
-- 11. Сид вопросов — данные теперь только в supabase/questions/bank.sql
-- ------------------------------------------------------------
-- Раньше здесь был insert 10 стартовых вопросов. На свежей базе он
-- выполнялся ДО bank.sql (см. порядок установки в README), а bank.sql
-- сверяет дубли по тексту вопроса (`on conflict (question) do nothing`)
-- — из-за этого для вопросов с одинаковым текстом, но разным порядком
-- вариантов, побеждала версия из этого сида, а не аудированная версия
-- из bank.sql (там порядок вариантов специально перемешан под ровное
-- распределение правильных ответов по позициям — см. 008). На проде
-- это не баг: там вопросы обновились позже отдельным UPDATE. Но на
-- свежей базе, поднятой строго по README, это тихо возвращало старый
-- перекос. Сид убран — bank.sql сам создаёт первые вопросы, и его
-- запуск README требует сразу после этой миграции.
